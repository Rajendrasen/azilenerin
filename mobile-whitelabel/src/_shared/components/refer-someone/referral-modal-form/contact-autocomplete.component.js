...
class ContactAutoComplete extends React.Component {
  ...
  getPhoneContacts = async () => {
    if (Platform.OS === 'ios') {
      ...
      if (this.state.showPolicyDisclosure) {
        ...
        if (granted == PermissionsAndroid.RESULTS.GRANTED) {
          ...
          Alert.alert(
            'Permission Denied',
            'Without this permission the app is unable to fetch the contacts from the device . please enable the permission from the settings for the app',
            [
              {
                text: 'CANCEL',
        ...
        //     console.log(">> error contact", res);
        // })
      } else {
        this.setState({showPolicyDisclosure: true});
      }
    }
  };
  filterModalContacts = () => {
    let {contactSearchValue} = this.state;
    let contacts = [...this.state.modalContacts];
    if (contactSearchValue) {
      return contacts.filter((ref) => {
        const contactName = ref ? `${ref.givenName} ${ref.familyName}` : null;
        if (contactName) {
          return (
            contactName
              .toLowerCase()
              .indexOf(contactSearchValue.toLowerCase()) > -1
          );
        }
      });
    }
    ...
    if (newContact) {
      return (
        <View>
          ...
          {this.state.showPolicyDisclosure && (
            <Modal
              visible={this.state.showPolicyDisclosure}
              transparent
              style={{
                flex: 1,
                height: '100%',
                  ...
                  <View
                    ...
                    <Text
                      ...
                      {getAppName().charAt(0).toUpperCase() +
                        getAppName().substring(1)}{' '}
                      app wants to access contact list directly from your phone
                      and save it to{' '}
                      {getAppName().charAt(0).toUpperCase() +
                        getAppName().substring(1)}{' '}
                      app server so that you can easily refer your friends
                      directly via contact list for jobs available on the
                      platform .
                    </Text>
                  </View>
                  <View
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        this.setState({showPolicyDisclosure: false})
                      }>
          ...
          <View style={{marginHorizontal: 10}}>
            <View style={styles.FormItemStyles}>
              ...
              ContactOptions.length ? (
                <View
                  ...
                  <Text style={{fontSize: 12, color: 'grey', padding: 7}}>
                    Refer an existing contact:
                  </Text>
                  <ScrollView
                    style={{
                      paddingLeft: 7,
                    ...
                    {ContactOptions.map((item) => (
                      <TouchableOpacity
                        key={item.id}
                        onPress={() => {
                          handleNewContact(() => this.onChangeText(item));
                        }}
            ...
            <View style={styles.FormItemStyles}>
              ...
              {getFieldDecorator('lastName', {
                ...
                <TextareaItem
                  ...
                  placeholderTextColor={COLORS.lightGray}
                  onFocus={() => this.setState({showSuggestion: false})}
                />,
              )}
            </View>
            {/* <View style={styles.FormItemStyles}>
            ...
            <View>
              <View
                ...
                {getAppName() == 'GoDaddy' &&
                this.props.currentUser?.company
                  ?.disableNewReferralSMSNotification == true ? null : (
                  <TouchableOpacity
                    style={{
                      padding: 8,
                      paddingHorizontal: 10,
            ...
            <Text
              ...
              {customTranslate('ml_ReferFromPhoneBook')}
            </Text>
                ...
                {this.state.modalContacts && this.state.modalContacts.length ? (
                  <View style={{flex: 1}}>
                    <TextInput
                      ...
                      placeholder="Search"
                      value={this.state.contactSearchValue}
                      onChangeText={(val) =>
                        this.setState(
                    ...
                    />
                    <ScrollView>
                      {_.sortBy(this.filterModalContacts(), [
                        'givenName',
                        'familyName',
                      ]).map((con) => (
                        <TouchableOpacity
              ...
              <View style={[styles.LabelStyles, styles.AutoCompleteZIndex]}>
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  ...
                  <TouchableOpacity
                    ...
                    <AntIcon name="closecircle" size={20} color="darkgrey" />
                  </TouchableOpacity>
                </View>
                {this.state.showSuggestion && (
                  <ScrollView
